...
class ReadRss:
    ...
    def insertData(self,title,date,url,description, source):
        ...
        query = "INSERT INTO `chandigarh`(`title`,`date`,`url`, `description`, `source`) VALUES(%s,%s,%s,%s,%s)"
       ...
       # mycursor.execute('insert into `bangalore`(`title`,`date`,`url`, `description`, `source`) values(%s, %s)', (id, formatted_date))
        print(type(mydate))
        thisdate = "%s"%(mydate)
        args=(title[9:-3],thisdate,url[9:-3],description[9:-3],source)
